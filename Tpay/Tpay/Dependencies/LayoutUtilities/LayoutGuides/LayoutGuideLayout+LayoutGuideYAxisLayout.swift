...
extension LayoutGuideLayout {
    ...
    final class LayoutGuideYAxisLayout: YAxisLayout {
        ...
        init(for view: LayoutGuide, anchor: LayoutAnchor.YAxis, invoker: Layout, container: ConstraintsContainer) {
        ...
        func equalTo(_ layoutGuide: LayoutGuide, _ anchor: LayoutAnchor.YAxis) -> ConstraintMultiplier {
            let constraint = LayoutConstraint(item: self.layoutGuide, itemAttribute: self.anchor.attribute, relation: .equal, target: layoutGuide, targetAttribute: anchor.attribute)
            return makePrioritizer(for: endorse(constraint))
        }
