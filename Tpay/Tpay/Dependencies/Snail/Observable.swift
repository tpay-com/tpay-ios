...
class Observable<T>: ObservableType {
    ...
    func subscribe(queue: DispatchQueue? = nil, onNext: ((T) -> Void)? = nil, onError: ((Error) -> Void)? = nil, onDone: (() -> Void)? = nil) -> Subscriber<T> {
    ...
    func on(_ queue: DispatchQueue) -> Observable<T> {
    ...
    func block(timeout: TimeInterval) -> Result<T?, Error> {
    ...
    func throttle(_ delay: TimeInterval) -> Observable<T> {
        let observable = Observable<T>()
    ...
    func debounce(_ delay: TimeInterval) -> Observable<T> {
        let observable = Observable<T>()
        let scheduler = Scheduler(delay)
    ...
    func skip(first: UInt) -> Observable<T> {
        let observable = Observable<T>()
        var count = first
    ...
    func take(first count: UInt) -> Observable<T> {
        let observable = Observable<T>()
        var taken = 0
    ...
    static func merge(_ observables: [Observable<T>]) -> Observable<T> {
    ...
    static func combineLatest<U>(_ input1: Observable<T>, _ input2: Observable<U>) -> Observable<(T, U)> {
    ...
    static func combineLatest<U, V>(_ input1: Observable<T>,
    ...
    static func combineLatest<U, V, K>(_ input1: Observable<T>,
                                              _ input2: Observable<U>,
    ...
    static func zip<U>(_ input1: Observable<T>, _ input2: Observable<U>) -> Observable<(T, U)> {
        let combined = Observable<(T, U)>()
