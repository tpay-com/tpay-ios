...
extension PayWithRatyPekaoViewController {
    ...
    final class ContentView: UIView, PinnableContentProvider {
        ...
        private let heading = Label.Builder(label: .H2())
            .set(text: Strings.payWithRatyPekaoSceneHeadline)
            .build()
        
        private(set) lazy var collectionView = CollectionView()
        
        private let bottomSection = BottomSection(with: .init(actionButtonTitle: Strings.payWithRatyPekaoSceneActionButtonTitle,
        ...
        private func fillPinnedContentSpace() {
            bottomSectionPlaceholder.layout
                ...
                .height.greaterThanOrEqualTo(pinnableContent, .height).with(constant: 50)
                .activate()
        }
    }
}
