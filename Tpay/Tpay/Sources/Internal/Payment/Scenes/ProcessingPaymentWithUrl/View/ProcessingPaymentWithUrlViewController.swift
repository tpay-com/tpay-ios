...
final class ProcessingPaymentWithUrlViewController: UIViewController {
    
    // MARK: - Properites
    
    let viewModel: ProcessingPaymentWithUrlViewModel
    
    private lazy var contentView = ContentView(successUrl: viewModel.successUrl, errorUrl: viewModel.errorUrl)
    
    // MARK: - Initialization

    init(with viewModel: ProcessingPaymentWithUrlViewModel) {
